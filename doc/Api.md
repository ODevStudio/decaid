...
| GET | `/api/v1/diagnostics/ble` | Versioned read-only BLE snapshot with wall/monotonic correlation, bounded service collection, logical peers, adapter, scan/watch ownership, reconnect policy, cache, and advertisement diagnostics | `ble_diagnostics_handler.dart` |
